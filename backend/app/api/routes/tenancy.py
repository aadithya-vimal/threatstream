...
async def get_tenancy_context(user: AuthenticatedUser = Depends(get_current_user)):
    return await TenancyService(user).context()


@router.post("/organizations", response_model=OrganizationBootstrapResponse, status_code=status.HTTP_201_CREATED)
async def create_organization(payload: OrganizationCreate, user: AuthenticatedUser = Depends(get_current_user)):
    return await TenancyService(user).create_organization(payload)
...
async def create_workspace(
    ...
    return await TenancyService(user).create_workspace(organization_id, payload)
...
async def list_teams(
    ...
    return await TenancyService(user).list_teams(workspace_id)
...
async def create_team(
    ...
    return await TenancyService(user).create_team(workspace_id, payload)
...
async def list_credential_metadata(
    ...
    return await TenancyService(user).list_credential_metadata(workspace_id)
...
async def store_credential(
    ...
    return await TenancyService(user).store_credential(workspace_id, provider_key, payload)
...
async def delete_credential(
    ...
    await TenancyService(user).delete_credential(workspace_id, provider_key)
    return None
